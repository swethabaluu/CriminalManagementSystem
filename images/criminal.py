...
class Criminal:
    def __init__(self,root):
        ...
        #criminalno
        lbl_criminal_no=Label(upper_frame,font=('arial',11,'bold'),text='Criminal No:',bg='white')
        ...
        female=Radiobutton(radio_frame_gender,text='Female',variable=self.var_gender,value='female',font=("arial",9,"bold"),bg='white')
        ...
        #pdf button
        btn_pdf = Button(button_frame, text='Export to PDF', command=self.export_to_pdf, font=("arial", 13, "bold"),width=14, bg='blue', fg='white')
        btn_pdf.grid(row=0,column=4,padx=3,pady=5)

        #barbutton
        btn_bar_chart = Button(button_frame, text='Display Bar Chart', command=self.plot_headers_bar_chart, font=("arial", 13, "bold"), width=18, bg='blue', fg='white')
        btn_bar_chart.grid(row=0, column=5, padx=3, pady=5)

      
       #background right side image
        img_crime=Image.open('images/police4.jpg')
        ...
        #searchframe
        search_frame=LabelFrame(down_frame,bd=2,relief=RIDGE,text='Criminal Table',font=('times new roman',11,'bold'),fg='red',bg='white' )
        ...
        combo_search_box=ttk.Combobox(search_frame,textvariable=self.var_com_search,font=("arial",11,"bold"),width=18)
        ...
        # search button
        btn_search = Button(search_frame, command=self.serch_data, text='Search', font=("arial", 13, "bold"), width=14, bg='blue', fg='white')
    ...
    def add_data(self):
         if self.var_case_id.get()=="":
              ...
              try:
                   conn=mysql.connector.connect(host='localhost',username='root',password='Student@123',database='management')  
    ...
    def update_data(self):
         if self.var_case_id.get()=="":
              ...
              try:
                   ...
                   if not conn:
                        conn = mysql.connector.connect(host='localhost', username='root', password='Student@123', database='management')
                        setattr(self, 'db_connection', conn)
                        my_cursor=conn.cursor()
    ...
    def delete_data(self):
         if self.var_case_id.get()=="":
              messagebox.showerror('Error','All Fields are required')
               ...
               try:
                   Delete=messagebox.askyesno('Delete','Are you sure to delete this record')
                   if Delete>0:
                        conn = mysql.connector.connect(host='localhost', username='root', password='Student@123', database='management')
                        my_cursor=conn.cursor()
...
#pdf
    def export_to_pdf(self):
        try:
            conn = mysql.connector.connect(host='localhost', username='root', password='Student@123', database='management')
            my_cursor = conn.cursor()
            my_cursor.execute('select Case_id, Criminal_name, gender, age, crimeType from criminal1')
            data = my_cursor.fetchall()
            conn.close()

            pdf_file_path = "criminal_records_table.pdf"
            ...
            table_data = [['Case ID', 'Criminal Name', 'Gender', 'Age', 'Crime Type']]  # Header row
            ...
            messagebox.showinfo("Success", "PDF generated successfully. Check 'criminal_records_table.pdf'")
        except Exception as es:
            messagebox.showerror('Error', f'Due To {str(es)}')

#barchart
    def plot_headers_bar_chart(self):
        try:
            conn = mysql.connector.connect(host='localhost', username='root', password='Student@123', database='management')
            my_cursor = conn.cursor()
            my_cursor.execute('select age, count(*) from criminal1 group by age')
            data = my_cursor.fetchall()
            conn.close()
